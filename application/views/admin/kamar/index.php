													...
													<td>
														<div class="row">
															...
															<a class="btn btn-danger btn-sm text-white ml-1" href="<?= base_url('admin/crud_kamar/h_kamar/');
																													echo $key->code_kamar . '/kamar' ?>"><i class="fa fa-trash"></i> </a>
					...
					<form name="f1z" method="post" action="<?= base_url('admin/crud_kamar/tb_kamar'); ?>">
						...
						<div class="row">
							...
							<div class="col">
								...
								<a href="javascript:void(0);" NAME="PENGAJAR" title="Klik Untuk Cari NAMA PENGAJAR" onClick='javascript:window.open("tkamarlaki","Ratting",
						"width=750,height=570,toolbar=1,status=1,");'>
									<input type="text" name="wk" id="wk" class="form-control" placeholder="Input Wali kamar" required>
								</a>
							</div>
						</div>
						<br>
	...
	<?php foreach ($kmr as $key) : ?>
		...
		<div class="modal fade" id="modaled<?= $key->code_kamar ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-lg" role="document">
				<div class="modal-content">
					<div class="modal-header bg-success">
						<h5 class="modal-title" id="exampleModalLabel">Form Edit Kamar</h5>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
					<div class="modal-body">
						...
						<form name="f1z" method="post" action="<?= base_url('admin/crud_kamar/ed_kamar'); ?>">
							...
							<div class="row">
								...
								<div class="col">
									<label for="foto">Wali Kamar</label>
									<a href="javascript:void(0);" NAME="PENGAJAR" title="Klik Untuk Cari NAMA PENGAJAR" onClick='javascript:window.open("ekamarlaki","Ratting",
						"width=750,height=570,toolbar=1,status=1,");'>
							...
							<div class="row">
								...
								<div class="col">
									...
									<select name="rayone" class="form-control" required>
										<option value="<?= $key->rayon ?>">-Jangan diganti bila tak ingin di rubah-</option>
										<option value="AT-Ikhlas">AL-Ikhlas</option>
										<option value="AT-Taubah">AT-Taubah</option>
										<option value="AL-Kautsar Selatan">AL-Kautsar Selatan</option>
										<option value="AL-Kautsar Utara">AL-Kautsar Utara</option>
								...
								<div class="col">
									...
									<select name="rke" class="form-control" required>
										<option value="<?= $key->ruang_kamar ?>">-Jangan di Rubah jika tak ingin di ganti-</option>
										<?php for ($i = 1; $i <= 15; $i++) { ?>
											<option value="<?= $i ?>"><?= $i ?></option>
										<?php } ?>
									</select>
